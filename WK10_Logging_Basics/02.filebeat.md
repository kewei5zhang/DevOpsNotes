...
Create filebeat_internal username and password

Go to Stack Management User and create the `filebeat_internal` user, and generate your own password. Then assign the `filebeat_write` role
![Alt text](images/filebeat_internal_user.png?raw=true)
...
export FILEBEAT_INTERNAL_PASSWORD=<your password> && docker compose -f docker-compose.yml -f extensions/filebeat/filebeat-compose.yml up
```
